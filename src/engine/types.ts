...
export type Episode = {
  ...
  weight: number;
  /** Множитель веса по силе соперника (M23, 26.09, из статьи о фланговых треугольниках): перегрузка
   *  фланга работает почти только против низкого блока, а выход через треугольник у своих ворот нужен
   *  против прессинга. Ключи — Strength ('weak' | 'even' | 'strong'), отсутствующий ключ = 1.
   *  Тип строкой, а не Strength, чтобы types.ts не зависел от conditions.ts. */
  weightBy?: { strength?: Record<string, number> };
  ...
  requires?: {
    ...
    /** Ліга (M17, 22.09): `top` — тільки вища (другий сезон, ми андердоги), без поля — будь-яка.
     *  Фільтрується при створенні матчу за conditions.league, планувальник цих епізодів у другій лізі не бачить. */
    league?: 'top' | 'second';
  };
  setup: string;
